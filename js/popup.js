...
const controlVolumeArrowCheckbox = document.querySelector('#control_volume_arrow');

// Load options values.
chrome.storage.sync.get({ enabled: true, controlAlwaysVisible: false, hideVideoInfo: false, controlVolumeWithArrows: false }).then(items => {
...
controlVolumeArrowCheckbox.addEventListener('change', e => {
    chrome.storage.sync.set({ controlVolumeWithArrows: controlVolumeArrowCheckbox.checked });
});
