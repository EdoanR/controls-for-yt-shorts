...
const localizeHtmlPage = function() {
    ...
    const attributes = ['', 'placeholder', 'title', 'value', 'innerHTML', 'aria-label'];
    ...
    for (let element of urlElements) {
        ...
        if (!attType) {
            console.log('[localize.js] Element with data-i18n-url without provinding the attribute data-i18n-type', element);
            continue;
        }

        let url = element.getAttribute(attType);
        if (!url) {
            console.log(`[localize.js] Attribute provided by data-i18n-type ("${attType}"), dont exist on the element`, element);
            continue;
        }
